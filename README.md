...
在 PCB 图片上按住鼠标左键拖出矩形，即可创建接口。选中接口后，在右侧填写：
...
点击已有导线后按 `Delete` 或 `Backspace` 可以删除连接。
...
- “导出图片”：生成高清 PNG 接线图，不受当前画布缩放比例影响
...
当前格式版本为 WireSketch `1.1.0`，分为两类文件：
...
程序仍兼容早期没有 Schema 标识，或缺少接口方向、节点旋转、翻转和缩放字段的描述文件。旧文件重新导出后会自动补齐 1.1 字段。
